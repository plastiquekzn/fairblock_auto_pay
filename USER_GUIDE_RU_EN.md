...
- локальный API-agent режим для демо с отдельным testnet private key;
...
### 10. Local API-agent режим

Vercel деплоит web UI. Локальный private-key API-agent не является частью статического Vercel frontend.

Для демо автономного агента локально:
...
- local API-agent demo mode with a dedicated testnet private key;
...
### 10. Local API-Agent Mode

Vercel deploys the web UI. The private-key API-agent is a local Node.js demo server and is not part of the static Vercel frontend.

For local agent demos:
